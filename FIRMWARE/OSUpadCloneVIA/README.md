...
eight WS2812 LEDs on PA5, keyboard, mouse, and consumer/media HID reports.
...
`PDF`, `OSL`, and `OSM`. Unicode, MIDI, audio, steno, tap dance, and
...
records migrate automatically to the 512-byte format. Writes are coalesced for
750 ms; wait one second after editing before unplugging. No boot strap, option
byte, read-out-protection, or bootloader area is modified.
